...
function srParam(value) {
    return String(value ?? '').replace(/[, \r\n]/g, encodeURIComponent);
}

function buildShadowrocketRules(rules, finalPolicy) {
    const result = [];
    for (const r of (rules || [])) {
        if (!r.enabled) continue;
        const target = r.action === 'block' ? 'REJECT' : 'DIRECT';
        switch (r.type) {
            case 'domain_suffix':  result.push(`DOMAIN-SUFFIX,${r.value.replace(/^\./, '')},${target}`); break;
            case 'domain_keyword': result.push(`DOMAIN-KEYWORD,${r.value},${target}`); break;
            case 'domain':         result.push(`DOMAIN,${r.value},${target}`); break;
            case 'geosite': {
                const custom = CUSTOM_GEOSITE_RULESETS[r.value];
                if (custom?.clashUrl) {
                    result.push(`RULE-SET,https://raw.githubusercontent.com/itdoginfo/allow-domains/main/Russia/outside-clashx.lst,${target}`);
                } else {
                    result.push(`GEOSITE,${r.value},${target}`);
                }
                break;
            }
            case 'geoip':          result.push(`GEOIP,${String(r.value).toUpperCase()},${target}`); break;
            case 'ip_cidr':        result.push(`IP-CIDR,${r.value},${target}`); break;
        }
    }
    result.push(`FINAL,${finalPolicy}`);
    return result;
}

function _buildShadowrocketVlessProxyForInbound(user, node, inbound) {
    const name = srName(_xrayInboundName(node, inbound));
    const host = node.domain || node.ip;
    const security = inbound.security || 'reality';
    const sni = inbound.realitySni && inbound.realitySni[0] ? inbound.realitySni[0] : host;
    const params = [
        `username=${srParam(user.xrayUuid)}`,
        'tls=true',
        'tls13=true',
        `sni=${srParam(sni)}`,
    ...
    const transport = inbound.transport || 'tcp';
    if (transport && transport !== 'tcp') params.push(`obfs=${srParam(transport)}`);
    if (inbound.flow) params.push(`vless-flow=${srParam(inbound.flow)}`);

    if (security === 'reality') {
        if (inbound.realityPublicKey) params.push(`reality-public-key=${srParam(inbound.realityPublicKey)}`);
        const shortId = (inbound.realityShortIds || ['']).find(id => id && id.length > 0) || '';
        if (shortId) params.push(`reality-short-id=${srParam(shortId)}`);
        if (inbound.fingerprint) params.push(`client-fingerprint=${srParam(inbound.fingerprint)}`);
    }

    return {
        name,
        line: `${name} = vless, ${host}, ${inbound.port || node.port || 443}, ${params.join(', ')}`,
    };
}

function _buildShadowrocketProxyLines(user, nodes) {
    const proxyLines = [];
    const proxyNames = [];
    ...
    nodes.forEach(node => {
        ...
        if (node.type === 'xray') {
            ...
            getXrayPublishedInbounds(node).forEach(inbound => {
                const built = _buildShadowrocketVlessProxyForInbound(user, node, inbound);
                proxyNames.push(built.name);
                proxyLines.push(built.line);
            });
        } else {
            getNodeConfigs(node).forEach(cfg => {
                const name = srName(`${node.flag || ''} ${node.name} ${cfg.name}`.trim());
                const params = [
                    `password=${srParam(`${user.userId}:${user.password}`)}`,
                    `sni=${srParam(cfg.sni || cfg.host)}`,
                    `skip-cert-verify=${cfg.hasCert ? 'false' : 'true'}`,
                ];
                if (cfg.obfs === 'salamander' && cfg.obfsPassword) {
                    params.push('obfs=salamander');
                    params.push(`obfs-password=${srParam(cfg.obfsPassword)}`);
                }
                proxyNames.push(name);
                proxyLines.push(`${name} = hysteria2, ${cfg.host}, ${cfg.port}, ${params.join(', ')}`);
            });
        }

        if (proxyNames.length > beforeIdx) {
            namesByNodeId.set(String(node._id), proxyNames.slice(beforeIdx));
        }
    });
    ...
    virtualSpecs.forEach(vnode => {
        ...
        for (const src of vnode._resolvedSources || []) {
            const names = namesByNodeId.get(String(src._id));
            if (names) sourceNames.push(...names);
        }
        if (sourceNames.length === 0) return;
        const name = srName(`${vnode.flag || ''} ${vnode.name}`.trim());
        const obs = (vnode.virtual && vnode.virtual.observatory) || {};
        const type = vnode.virtual?.strategy === 'random' ? 'load-balance' : 'url-test';
        const interval = parseDurationSeconds(obs.interval || '1m') || 60;
        groupLines.push(`${name} = ${type}, ${sourceNames.join(', ')}, url=${obs.destination || 'https://www.gstatic.com/generate_204'}, interval=${interval}`);
        virtualNames.push(name);
    });

    return { proxyLines, proxyNames, groupLines, virtualNames };
}

function generateShadowrocketConf(user, nodes, routing) {
    const { proxyLines, proxyNames, groupLines, virtualNames } = _buildShadowrocketProxyLines(user, nodes);
    const defaultPolicy = virtualNames[0] || 'Proxy';
    const selectNames = [...virtualNames, ...proxyNames];
    const proxyGroupLines = [];
    if (selectNames.length > 0) {
        proxyGroupLines.push(`Proxy = select, ${selectNames.join(', ')}`);
    }
    proxyGroupLines.push(...groupLines);

    const rules = (routing && routing.enabled)
        ? buildShadowrocketRules(routing.rules, defaultPolicy)
        : [`FINAL,${defaultPolicy}`];
...
function generateSubscriptionData(user, nodes, format, userAgent, happProviderId = '', routing = null) {
    ...
    switch (format) {
        case 'shadowrocket':
            content = generateShadowrocketConf(user, nodes, routing);
            break;
        case 'clash':
        case 'yaml':
