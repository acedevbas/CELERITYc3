...
- \`/ws/terminal/{nodeId}\`, \`/ws/logs\`, \`/ws/broadcast\` — WebSocket-каналы только для админ-сессии (cookie).
        ...
        operations: {
            ...
            'POST /nodes': {
                ...
                description: `Создаёт **Hysteria**, **Xray**, **AmneziaWG** или **Virtual** (балансировщик) ноду. \`statsSecret\` генерируется сервером.

- \`type=hysteria\` (по умолчанию) / \`type=xray\` / \`type=amneziawg\` — обязателен \`ip\`. 409, если на том же IP уже есть нода того же \`type\`.
...
Требуется скоуп \`nodes:write\`.`,
            ...
            'GET /nodes/check-ip': {
                ...
                description: 'Возвращает реальные protocol-ноды на том же IP. Используется интерфейсом при добавлении ноды. Виртуальные ноды не имеют IP и в выдаче не появляются.',
            },
            'GET /nodes/{id}/status': {
